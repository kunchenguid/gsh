...
type releasePleasePackage struct {
	...
	ExtraFiles  extraFiles `json:"extra-files"`
}

// extraFiles accepts both release-please forms:
//
//	"extra-files": ["flake.nix"]
//	"extra-files": [{"type": "generic", "path": "flake.nix"}]
type extraFiles []string

func (e *extraFiles) UnmarshalJSON(data []byte) error {
	var strs []string
	...
	if err := json.Unmarshal(data, &objs); err != nil {
		return fmt.Errorf("extra-files: want string list or {path} object list: %w", err)
	}
